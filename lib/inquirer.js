    ...
    connection.query("SELECT CONCAT(employee.first_name, \" \", employee.last_name) AS employee_name FROM employee", function (err, data) {
        ...
        inquirer
            .prompt([
                ...
                connection.query("DELETE FROM employee WHERE CONCAT(employee.first_name, \" \", employee.last_name) = ?", answers.employee, function (err, data) {
                    if (err) throw err;
                    console.log(`${answers.employee} is removed from employee list.`);
                    viewAllEmployees();
                });
